...
from celeritas.constants import client, LAMPORTS_PER_SOL, WRAPPED_SOL
...
class UserDB:
    ...
    def update_user_positions(self, id: int, update_holdings=False, get_prices=False) -> dict:
        ...
        for token, balance in holdings.items():
            position = {k: 0 for k in ["balance", "avg_entry_sol", "avg_entry_usd", "n_buys", "n_sells", "realized_pnl_usd", "realized_pnl_sol", "realized_pnl_percentage_usd", "realized_pnl_percentage_sol", "unrealized_pnl_usd", "unrealized_pnl_sol", "unrealized_pnl_percentage_usd", "unrealized_pnl_percentage_sol"]}
            ...
            for tx in sorted(filter(lambda x: x['mint'] == token, transactions), key=lambda x: x['timestamp']):
            ...
            if total_sold["tokens"] > 0:
                ...
                for currency in ["sol", "usd"]:
                    position[f'realized_pnl_{currency}'] = total_sold[currency] - (total_sold["tokens"] * position[f'avg_entry_{currency}'])
                    position[f'realized_pnl_percentage_{currency}'] = (avg_sell_price[currency] / position[f'avg_entry_{currency}'] - 1) * 100 if position[f'avg_entry_{currency}'] else 0
            if get_prices and token in current_prices:
                ...
                for currency, price in [("usd", current_price_usd), ("sol", current_price_sol)]:
                    position[f'unrealized_pnl_{currency}'] = (price - position[f'avg_entry_{currency}']) * current_token_balance
                    position[f'unrealized_pnl_percentage_{currency}'] = ((price / position[f'avg_entry_{currency}']) - 1) * 100 if position[f'avg_entry_{currency}'] else 0
...
class TokenDB:
    ...
    async def get_prices(self, mints: List[str], add_missing=False) -> dict:
        ...
        tokens = self.tokens.find(
            {"_id": {"$in": mints}}, {"price_dollars": 1, "refresh_timestamp": 1}
        )
        for token in tokens:
    ...
    async def update_pump_fun_token(self, mint: str, token=None):
        ...
        token["price_dollars"] = (
            token["pump_fun_data"]["bonding_curve_price_sol"] * sol_dollar_value()
        )
        token["market_cap_dollars"] = (
            token["supply"] * token["price_dollars"] / 10 ** token["decimals"]
        )
    ...
    async def update_price_history(self, token: dict, new_price: float):
        ...
        token["price_history"] = [
            entry for entry in token["price_history"]
            if entry["timestamp"] > current_time - 25 * 60 * 60 
        ]
        return token
