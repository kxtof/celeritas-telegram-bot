...
# Define new states for the buy conversation
BUY_TOKEN, BUY_TOKEN_NEW, REFRESH_TOKEN_BUY, CLOSE_TOKEN_BUY, CUSTOM_BUY_AMOUNT, AMOUNT_TO_BUY, AMOUNT_TO_BUY_CUSTOM, SET_CUSTOM_BUY_SLIPPAGE, SET_BASE_BUY_SLIPPAGE, EXECUTE_BUY, CUSTOM_BUY_SLIPPAGE = range(100, 100+11)
...
async def generate_token_buy_text(user, token, options):
    ...
    bottom_line = f"{nfpf(token_out_estimate)} {t_symbol} | ${nfpf(token_out_dollar_value_estimate, underline=True)}"
...
async def set_option(update: Update, context: ContextTypes.DEFAULT_TYPE, option: str) -> int:
    ...
    context.user_data[f"buy_message_options_{token_mint}"][option] = amount if option == "amount" else int(amount)
...
async def set_custom_buy_slippage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    return await prompt_custom_input(update, context, "slippage", "Please enter your custom slippage %:", CUSTOM_BUY_SLIPPAGE)

async def set_custom_amount_to_buy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    return await prompt_custom_input(update, context, "amount", "Please enter your custom buy amount in SOL:", CUSTOM_BUY_AMOUNT)
...
async def process_custom_input(update: Update, context: ContextTypes.DEFAULT_TYPE, option: str) -> int:
    ...
    try:
        ...
        reply_markup = await generate_token_buy_keyboard(
            user, token, options
        )
...
async def execute_buy_order(context: ContextTypes.DEFAULT_TYPE, user, mint: str, amount: float, slippage: int, message=None):
    transact = Transact(user.wallet_secret, fee_sol=user.settings.priority_fee)
    quote = await transact.buy(
        mint,
        amount,
    ...
    txs = await transact.construct_and_send(
        quote,
        fee=(0.9 if user.referrer else 1) * amount * 0.01  # add transaction fee
    )
    text = (
    ...
    if txs:
        await schedule_tx_update(context, message.chat_id, message.message_id, user.id, txs, mint, user.wallet_public)
    return txs
...
async def is_token_mint(text):
    try:
        ...
        return False
    # Pubkey is valid
    if await tokendb.get_token(text):
        return True
    # Token is not in db, check RPC if pubkey is a token mint
    try:
...
token_buy_conv_handler = ConversationHandler(
    ...
    states={
        ...
        CUSTOM_BUY_AMOUNT: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, custom_buy_amount_input)
        ],
        CUSTOM_BUY_SLIPPAGE: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, custom_buy_slippage_input)
        ],
        CONFIRM_BUY: [
