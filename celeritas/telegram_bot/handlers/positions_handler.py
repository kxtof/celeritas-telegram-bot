...
async def sell_token(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query:
        await query.answer()
    db.get_attribute(user_id, "wallet_public")
...
async def generate_sell_keyboard(user, page):
    ...
    tokens_by_amount = [
        t[0] for t in sorted(user.holdings.items(), key=lambda x: -x[1])
    ]
    start = page * TOKENS_PER_PAGE
    end = start + TOKENS_PER_PAGE
    tokens = tokendb.get_tokens(tokens_by_amount[start:end])
    ...
    if page > 0:
        navigation_buttons.append(
            InlineKeyboardButton("Previous", callback_data=str(PREV_PAGE))
        )
    if end < len(tokens_by_amount):
        navigation_buttons.append(
            InlineKeyboardButton("Next", callback_data=str(NEXT_PAGE))
        )
...
async def refresh_sell_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    tokendb.update_price(tokens_by_amount[start:end])
...
positions_conv_handler = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(sell_menu_new, pattern="^" + str(SELL_MENU) + "$")
    ],
    states={
        SELL_MENU: [
            CallbackQueryHandler(
                refresh_sell_menu, pattern="^" + str(REFRESH_SELL_MENU) + "$"
            ),
            CallbackQueryHandler(
                close_sell_menu, pattern="^" + str(CLOSE_SELL_MENU) + "$"
            ),
            CallbackQueryHandler(
