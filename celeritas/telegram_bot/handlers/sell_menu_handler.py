...
async def generate_menu_keyboard(
    user, tokens, page, last, action_type
...
async def get_paginated_tokens(user, page, tokens_per_page, action_type):
    token_prices = await tokendb.get_prices(list(user.holdings.keys()))
    # Sort based on dollar value of holding while filtering out tokens without sufficient min_pos_value
    tokens_by_value = [t for t, h in sorted(user.holdings.items(), key=lambda x: -x[1] * token_prices.get(x[0], 0))
...
async def generate_token_text(user, token, token_info, sol=True):
    token_url = f"https://pump.fun/{token}" if token_info['is_pump_fun'] and not token_info['pump_fun_data']['bonding_curve_complete'] else f"https://dexscreener.com/solana/{token}?maker={user.wallet_public}"
...
async def get_tokens(tokens):
    sol = {'mint': 'SOL', 'symbol': 'SOL', 'price_dollars': sol_dollar_value(), 'is_pump_fun': False} if 'SOL' in tokens else None
    tokens_info = {'SOL': sol} if sol else {}
    tokens_info.update(await tokendb.get_tokens([token for token in tokens if token != 'SOL']))
    return tokens_info
...
async def token_menu(
    ...
    reply_markup = await generate_menu_keyboard(user, tokens_info, page, last=has_more, action_type=action_type)
...
async def refresh_sell_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_id = update.effective_user.id
    db.update_user_holdings(user_id)
    user = db.get_user(user_id)
    ...
    await tokendb.update_price(tokens_by_amount[start:end])
...
sell_menu_conv_handler = ConversationHandler(
    ...
    states={
        SELL_MENU: [
            ...
            CallbackQueryHandler(lambda update, context: sell_menu_page(update, context, direction=1), pattern="^" + str(NEXT_PAGE) + "$"),
            CallbackQueryHandler(lambda update, context: sell_menu_page(update, context, direction=-1), pattern="^" + str(PREV_PAGE) + "$"),
...
withdraw_menu_conv_handler = ConversationHandler(
    ...
    states={
        WITHDRAW_MENU: [
            ...
            CallbackQueryHandler(lambda update, context: withdraw_menu_page(update, context, direction=1), pattern="^" + str(NEXT_PAGE) + "$"),
            CallbackQueryHandler(lambda update, context: withdraw_menu_page(update, context, direction=-1), pattern="^" + str(PREV_PAGE) + "$"),
